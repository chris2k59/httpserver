...
void handle_files_request(int fd) {
  ...
  long unsigned bSize = strlen(data);
  ...
  if(stat(path, &fileStat) >=0 ) 
  {
    if(S_ISREG(fileStat.st_mode) != 0)
    {
     ...
     {
      char *dirlist = Dirlist(request->path);
      if(strcmp(dirlist,"index.html") ==0)
      {
        status = 200;
        mime_type = http_get_mime_type(dirlist);
        char *path1 = append(path,"/index.html");
        data = buffer(path1, &bSize);
